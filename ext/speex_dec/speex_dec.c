...
Init_speex_dec(void)
{
  rb_mSpeexDec = rb_define_module("SpeexDec");

  VALUE mod = rb_define_module_under(rb_mSpeexDec, "Internal");

  rb_define_singleton_method(mod, "to_wav", to_wav, 2);
}

VALUE to_wav(VALUE mod, VALUE src_path, VALUE dest_path)
{
  FILE      *fpInput;
  FILE      *fpOutput;
  ...
  fpInput = fopen(StringValueCStr(src_path), "rb");
  ...
  fpOutput = fopen(StringValueCStr(dest_path), "wb");
